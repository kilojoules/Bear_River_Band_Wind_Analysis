...
NREL.dat$day <- as.numeric(format(NREL.dat$time,format = '%w'))

# map from 60 feet to 170 feet
NREL.dat$avg.speed.150m=NREL.dat$avg.speed*(140/60)^(1/7)
NREL.dat$pow <- wpcm(NREL.dat$avg.speed.150m,3)
...
for (i in 1:12)
{
  ...
  if(length(m$time)>0)
  {
    ...
    avg.months$power.ext.Aeolos[i] <- mean(wpcm(m$avg.speed.150m,2))
    avg.months$power.ext.Excel[i] <- mean(wpcm(m$avg.speed.150m,3))
    avg.months$power.ext.Northern[i] <- mean(wpcm(m$avg.speed.150m,4))
  }
}
write.csv(avg.months,file='~/Desktop/nrelFork.csv',row.names=FALSE)
